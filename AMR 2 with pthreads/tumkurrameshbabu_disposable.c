...
#define NS_PER_US 100000000
#include "simple_amr2.h"
...
void *update_DSV(void *var){
int curr_box = *((int*) var);
int contact_dist;
double sum, average;
int j, temp_box;
...
void converLoop(int numThreads)
{
	...
	*param = i;
	//printf("inside for");
        index = i % numThreads;
        p = pthread_create(&threads[*param], NULL, update_DSV, (void *)param);
	//printf("Thread Number= %d",index);
	//printf(" Thread id===\n");
        //if (index == numThreads - 1 ||((index!= numThreads)&&(index == (num_box - 1)%numThreads))) {
        //}
    }
...
double diff=0;
clock_t clock_time, clock_time1;
time_t timer_start;
time_t timer_end;
...
//printf("inside while");
// push this to convergence loop --- update_DSV();
//  print_data();
// push this also to convergence loop --- copy_data();
//printf("entering conver loop");

//clock_time = clock();
converLoop(numThreads);

//clock_time1 += clock()- clock_time;
